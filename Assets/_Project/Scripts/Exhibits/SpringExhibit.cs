...
namespace ScienceMuseum.Exhibits
{
    public class SpringExhibit : ExhibitBase
    {
        ...
        [Tooltip("Точка крепления пружины сверху")]
        ...
        protected override void Awake()
        {
            ...
            _parameters = new[]
            {
                new ExhibitParameter("Масса m", "кг", 0.1f, 10f,
                    () => mass, v => Mass = v, decimals: 2),
                new ExhibitParameter("Жёсткость k", "Н/м", 5f, 200f,
                    () => stiffness, v => Stiffness = v, decimals: 1),
                new ExhibitParameter("Трение c", "", 0f, 5f,
                    () => damping, v => Damping = v, decimals: 2),
                new ExhibitParameter("Начальное смещение x0", "м", -0.5f, 0.5f,
                    () => initialDisplacement, v => InitialDisplacement = v, decimals: 2),
            };
        ...
        {
            float period = 2f * Mathf.PI * Mathf.Sqrt(mass / stiffness);
            float frequency = 1f / period;
            float omega = Mathf.Sqrt(stiffness / mass);

            return
                "<b>Формула периода:</b>\n" +
                $"  T = 2π·√(m/k) = 2π·√({mass:F2}/{stiffness:F1})\n" +
